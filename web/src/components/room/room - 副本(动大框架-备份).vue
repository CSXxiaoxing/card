<template> 
    ...
    <div id='room'>
            ...
            <dl>
                ...
                <dd>
                    ...
                    <span>{{init.ForT == 1 ? init.totalFen: win.fen}}</span>
                     &nbsp;&nbsp;&nbsp; 
                    房号：<span>{{$store.state.idRoom.room_id}}</span>
                </dd>

                <dd  @click='init.ForT == 1 ? varRoom() : ""'><i></i></dd>
        ...
        <div class='roomMain'>
            <div class='left'>
                <ul>
                    ...
                    <li  
                    v-for = '(data, index) in ($store.state.data.Zlist == "" ? 4 : $store.state.data.Zlist)' 
                    v-if='index <= 5' :key='data.zn_member_id'>
                        <div v-show='data <= 4' class='konwei'>
                        </div>

                        <div v-if='data.zn_member_name != null'>
                        ...
                        <p v-if='data.zn_member_name != null'>{{data.zn_member_name}}</p>
                    </li>
            ...
            <div class='center'>
                ...
                <ul class='ka7' id='room_UL'>
                    <li 
                    v-for='(data,index) in 7' 
            ...
            <div class='right'>
                ...
                <ul @click='speak'>
                    <li
                    v-for='player in (roomData999[0] == "" ? $store.state.data.Room["id"] : roomData999)' 
                    v-if='fanzhuID != player.zn_member_id' 
                    :key='player.zn_member_id'>
                    ...
                    <li v-for='a in 5-($store.state.data.Room["id"] == undefined ? 0 : $store.state.data.Room["id"].length)'>
        ...
        <div class='control'>
            <!-- 上下庄 -->
            <span @click="applyOn(99)" :class='[(init.ForT == 1 ? "zhuan" : ""),($store.state.data.apptype == 1 ? "win" : ""),("control_left")]'>
                {{$store.state.data.apptype == 1 ? "取消上庄" : "申请上庄"}}
            </span>
            <!-- 筹码 -->
            <span class='control_center' @touchend='liType = -1,srcImgStyle="z-index:-999;"'>
                <img :src="`src/image/c${true ? 'c' : 'h'}01.png`"  @touchmove='true ? move() : false'/>
                <img :src="`src/image/c${true ? 'c' : 'h'}02.png`"  @touchmove='true ? move() : false'/>
                <img :src="`src/image/c${true ? 'c' : 'h'}03.png`"  @touchmove='true ? move() : false'/>
                <img :src="`src/image/c${true ? 'c' : 'h'}04.png`"  @touchmove='true ? move() : false'/>
                <img :src="`src/image/c${false ? 'c' : 'h'}05.png`" @touchmove='false ? move() : false'/>
            </span>
    ...
    export default {
        ...
        methods: {
            move(ev){   // 拖动筹码
                ...
                var kaNum = this.$store.state.idRoom.cardFn;          // 玩几牌的
            ...
            gameStyle(){       // 游戏流程--开始(1)
                ...
                var minGrade = Number(this.$store.state.idRoom.minGrade); // 最低上分分数
                ...
                if(Etxt == 1){
                    if(zPoints == undefined){
                        noZ('请检查有无庄家')
                    } else if(Number(zPoints) < minGrade){
